...
class TestBlockDevice(BlockdevTestcase):
    ...
    def test_format_bytes(self):
        ...
        for unit in units:
            ...
            e = cm.exception
            log.debug(
                "%s raised on format_bytes() with unit %r: %s",
        ...
        for bytes_ in (None, 'bla'):
            ...
            e = cm.exception
            log.debug(
                "%s raised on format_bytes() with value %r: %s",
        ...
        values = (
            ...
            ((A_KILO * A_KILO * A_KILO * A_KILO * A_KILO), 'GiB', (
                A_KILO * A_KILO), float(A_KILO * A_KILO)),
        )
        ...
        for tpl in values:
            ...
            log.debug(
                "Converting %r into %r, expected result: %r.",
                val, unit, exp_result)
            ...
            log.debug(
                "Converting %r into %r as float, expected result: %r.",
                val, unit, exp_result_f)
            result_f = format_bytes(val, unit, True)
            log.debug("Got converted float result: %r", result_f)
            msg = (
                "Failed formating bytes %r into %r as float, "
                "expected result: %r, got result: %r") % (
    ...
    def test_size_to_sectors(self):
        ...
        for unit in units:
            ...
            e = cm.exception
            log.debug(
                "%s raised on size_to_sectors() with unit %r: %s",
                e.__class__.__name__, unit, e)
        ...
        for tpl in values:
            ...
            log.debug(
                "Converting %d %s into sectors of %r Bytes, expected result: %r.",
    ...
    def test_existing(self):
        ...
        log.info(
            "Testing of a BlockDevice object of the existing block device %r.",
            devname)
    ...
    def test_statistics(self):
        ...
        log.info(
            "Testing of getting statistics of the existing block device %r.",
            devname)
    ...
    def test_fuser(self):
        ...
        try:
            ...
            self.assertGreaterEqual(
                len(pids), 1, (
                    "Method opened_by_processes() "
                    "should return a list with one element."))
            self.assertIsInstance(
                pids[0], int, (
                    "The elements of the result list "
                    "of opened_by_processes() shoul be integers."))
            ...
            e = cm.exception
            log.debug(
                "%s raised on opened_by_processes() with not existing file %r: %s",
